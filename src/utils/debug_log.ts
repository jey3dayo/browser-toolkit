...
/**
 * デバッグログをファイルに自動保存（Downloads フォルダ）
 */
export async function autoSaveDebugLogs(): Promise<string> {
  const logs = await getDebugLogs();
  const content = logs
    .map(
      (log) =>
  ...
  // Service Worker環境ではURL.createObjectURLが使えないため、data URLを使用
  const dataUrl = `data:text/plain;charset=utf-8,${encodeURIComponent(content)}`;
  const filename = "browser-toolkit-debug.log";

  const downloadId = await chrome.downloads.download({
    url: dataUrl,
    filename,
    saveAs: false, // 自動保存（確認ダイアログなし）
    conflictAction: "overwrite", // 既存ファイルを上書き
  });
