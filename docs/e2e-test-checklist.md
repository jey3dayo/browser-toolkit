...
- [ ] Google で任意のキーワードを検索する
- [ ] 検索結果カードにマウスをホバーし、遮断アイコンをクリック
- [ ] ダイアログでルールを追加する
- [ ] **期待**: 対象の検索結果が非表示になる

#### 9.2 件数バーの表示トグル

- [ ] 9.1 実施後、ページ上部（または結果一覧付近）の非表示件数バーを確認
- [ ] **期待**: 非表示にした件数が表示される
- [ ] バーの表示トグルを操作し、非表示にした結果を一時的に再表示できることを確認

#### 9.3 解除が即時に効く（reload なし）

- [ ] 9.1 で追加したルールの対象結果にマウスをホバーし、遮断アイコンをクリックする
- [ ] ダイアログで「解除する」をクリックする
- [ ] **ページを再読み込みせずに**、その場で結果カードの表示を確認する
- [ ] **期待**: 再読み込みしなくても、解除した対象の検索結果がその場で再び表示される

#### 9.4 解除がページ再読み込み後も持続する

- [ ] 9.3 の解除操作を行った状態でページを再読み込みする
- [ ] **期待**: 再読み込み後も解除が維持され、対象の検索結果が表示されたままになる

#### 9.5 ページ再読み込み後もルールが残る

- [ ] ブロックルールを追加した状態でページを再読み込みする
- [ ] **期待**: 再読み込み後も同じルールでブロックが適用される

#### 9.6 スクロールで追加読み込みされた結果への適用

- [ ] ブロック対象のサイトを含むキーワードで検索する
- [ ] ページ下部までスクロールして追加の検索結果を読み込む
- [ ] **期待**: 追加読み込みされた結果にも既存ルールが適用され、対象が非表示になる

#### 9.7 popup pane からのルール編集・削除

- [ ] Popup → **検索ブロックリスト** タブを開く
- [ ] 既存ルールを編集する（パターンを変更）
- [ ] 既存ルールを削除する
- [ ] **期待**: 編集・削除がすぐに検索結果ページの表示へ反映される
