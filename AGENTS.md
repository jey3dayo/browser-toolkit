...
耐久的な知識を読む・更新するときは、まずこの表で正本を決めてください。実体となる workflow、source routing、SSoT 表は `AGENTS.md` に集約し、詳細な仕様や運用ルールは対象ごとの正本に寄せます。

| 対象 | 正本 | 用途 |
| --- | --- | --- |
| Agent runtime entrypoint | `AGENTS.md` | agent runtime が最初に読む入口。実体を `AGENTS.md` に集約 |
| Agent workflow / source routing / SSoT table | `AGENTS.md` | 開発時の高レベルルール、正本表、entrypoint docs のルーティング |
...
| Code organization / placement | `AGENTS.md`, `docs/architecture.md`, source tree | ディレクトリ構成、entrypoint責務、feature placement、命名規則 |
