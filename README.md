...
- 検索結果ブロックリスト（Google 検索結果から指定サイトを非表示）
...
- Google 検索結果から指定サイトを非表示にできます（uBlacklist 相当）
- 対応検索エンジン: Google のみ（v1）
- 使い方: 検索結果カードにマウスを乗せると遮断アイコンが表示され、クリックすると
  ダイアログからルールを追加できます。以後そのサイトの結果は表示されません
- ルール構文: match pattern のサブセット（`*://*.example.com/*`、`example.com`、
  `*://example.com/path/*`）。**正規表現は v1 未対応**（入力すると明示エラーになります）
- 保存先は `chrome.storage.local` のため、**デバイス間では同期されません**
- 解除: 件数バーでは非表示にした結果の表示/非表示を切り替えられます。ルールの解除は、検索結果カードに
  ホバーしてダイアログを開くか、ポップアップ → **検索ブロックリスト** タブから行えます
...
ポップアップは小さなウィンドウなので日常的に使う 5 機能だけを載せ、オプションページは全機能
（日常操作の 5 機能 + 管理系）をレールに並べたフル画面として使います。
...
- Node `24以上`
- pnpm（バージョンは `packageManager` / `mise.toml` に従う）
